...
class coin_date(Base):
    ...
    coin_type = Column(String(255), primary_key = True, unique = False, nullable = False)
    unix_date = Column(Integer, primary_key = True, unique = False, nullable = False)
    entry_id = Column(Integer, unique = True, nullable = False, autoincrement=True)
    #relationships
    tax_volume_r = relationship('tx_volume', backref='coin_date', lazy=True)
    adjusted_tax_volume_r = relationship('adjusted_tx_volumne', backref='coin_date', lazy=True)
    ...
    avg_difficulty_r = relationship('median_difficulty', backref='coin_date', lazy=True)
...
class exchange_volume: #integer
    __tablename__="exchange_volume"
    entry_id = Column(Integer, ForeignKey('coin_date.entry_id'), primary_key=True)
    value = Column(Integer, nullable=False)

class realized_cap: #float
